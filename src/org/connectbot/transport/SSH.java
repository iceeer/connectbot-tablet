...
public class SSH extends AbsTransport implements ConnectionMonitor,
	...
	static {
		hostmask = Pattern.compile("^(.+)@([0-9a-z.-]+)(:(\\d+))?$",
	...
	private static final int conditions = ChannelCondition.STDOUT_DATA
			| ChannelCondition.STDERR_DATA | ChannelCondition.CLOSED
	...
	public class HostKeyVerifier implements ServerHostKeyVerifier {
		public boolean verifyServerHostKey(String hostname, int port,
			...
			switch (hosts.verifyHostkey(matchName, serverHostKeyAlgorithm,
					serverHostKey)) {
			case KnownHosts.HOSTKEY_IS_OK:
				bridge.outputLine(manager.res.getString(
						R.string.terminal_sucess, algorithmName, fingerprint));
				return true;

			case KnownHosts.HOSTKEY_IS_NEW:
				// prompt user
				bridge.outputLine(manager.res.getString(
						R.string.host_authenticity_warning, hostname));
				bridge.outputLine(manager.res.getString(
						R.string.host_fingerprint, algorithmName, fingerprint));

				result = bridge.promptHelper
						.requestBooleanPrompt(null, manager.res
								.getString(R.string.prompt_continue_connecting));
				if (result == null)
					return false;
				if (result.booleanValue()) {
					// save this key in known database
					manager.hostdb.saveKnownHost(hostname, port,
							serverHostKeyAlgorithm, serverHostKey);
				}
			...
			case KnownHosts.HOSTKEY_HAS_CHANGED:
				String header = String
						.format("@   %s   @",
								manager.res
										.getString(R.string.host_verification_failure_warning_header));
				...
				bridge.outputLine(manager.res
						.getString(R.string.host_verification_failure_warning));
				bridge.outputLine(border);
				...
				// Users have no way to delete keys, so we'll prompt them for
				// now.
				result = bridge.promptHelper
						.requestBooleanPrompt(null, manager.res
								.getString(R.string.prompt_continue_connecting));
	...
	private void authenticate() {
		...
		try {
			...
			if (!pubkeysExhausted
					...
					&& connection.isAuthMethodAvailable(host.getUsername(),
							AUTH_PUBLICKEY)) {

				// if explicit pubkey defined for this host, then prompt for
				// password as needed
				...
				if (pubkeyId == HostDatabase.PUBKEYID_ANY) {
					...
					for (Entry<String, KeyHolder> entry : manager.loadedKeypairs
							.entrySet()) {
						if (entry.getValue().bean.isConfirmUse()
								&& !promptForPubkeyUse(entry.getKey()))
							continue;

						if (this.tryPublicKey(host.getUsername(),
								entry.getKey(), entry.getValue().trileadKey)) {
							finishConnection();
							break;
						}
					}
					...
					bridge.outputLine(manager.res
							.getString(R.string.terminal_auth_pubkey_specific));
					// use a specific key for this host, as requested
					PubkeyBean pubkey = manager.pubkeydb
							.findPubkeyById(pubkeyId);

					if (pubkey == null)
						bridge.outputLine(manager.res
								.getString(R.string.terminal_auth_pubkey_invalid));
				...
				// this auth method will talk with us using InteractiveCallback
				// interface
				// it blocks until authentication finishes
				bridge.outputLine(manager.res
						.getString(R.string.terminal_auth_ki));
				interactiveCanContinue = false;
				if (connection.authenticateWithKeyboardInteractive(
						host.getUsername(), this)) {
					finishConnection();
				} else {
					bridge.outputLine(manager.res
							.getString(R.string.terminal_auth_ki_fail));
				}
			} else if (connection.isAuthMethodAvailable(host.getUsername(),
					AUTH_PASSWORD)) {
				if (host.getPassword() != null
						&& !HostBean.NOSAVE.equals(host.getPassword())
						&& connection.authenticateWithPassword(
								host.getUsername(), host.getPassword())) {
					finishConnection();
				} else {
					bridge.outputLine(manager.res
							.getString(R.string.terminal_auth_pass));
					String password = bridge
							.getPromptHelper()
							.requestStringPrompt(
									null,
									manager.res
					...
					if (password != null
							&& connection.authenticateWithPassword(
									host.getUsername(), password)) {
						if (!HostBean.NOSAVE.equals(host.getPassword())) {
							host.setPassword(password);
							manager.hostdb.saveHostPassword(host.getId(),
									host.getPassword());
						}
						finishConnection();
					} else {
						bridge.outputLine(manager.res
								.getString(R.string.terminal_auth_pass_fail));
					}
				}
			} else {
				bridge.outputLine(manager.res
						.getString(R.string.terminal_auth_fail));
			}
	 ...
	 * @throws IOException
	 */
	private boolean tryPublicKey(PubkeyBean pubkey)
		...
		if (manager.isKeyLoaded(pubkey.getNickname())) {
			...
			Log.d(TAG, String.format(
					"Found unlocked key '%s' already in-memory",
					pubkey.getNickname()));
			...
			// otherwise load key from database and prompt for password as
			// needed
			String password = null;
			if (pubkey.isEncrypted()) {
				password = bridge.getPromptHelper().requestStringPrompt(
						null,
						manager.res.getString(R.string.prompt_pubkey_password,
								pubkey.getNickname()));
			...
			if (PubkeyDatabase.KEY_TYPE_IMPORTED.equals(pubkey.getType())) {
				...
				trileadKey = PEMDecoder.decode(
						new String(pubkey.getPrivateKey()).toCharArray(),
						password);
			} else {
				...
				try {
					...
					String message = String
							.format("Bad password for key '%s'. Authentication failed.",
									pubkey.getNickname());
					Log.e(TAG, message, e);
		...
		return tryPublicKey(host.getUsername(), pubkey.getNickname(),
				trileadKey);
	}

	private boolean tryPublicKey(String username, String keyNickname,
			Object trileadKey) throws IOException {
		// bridge.outputLine(String.format("Attempting 'publickey' with key '%s' [%s]...",
		// keyNickname, trileadKey.toString()));
		boolean success = connection.authenticateWithPublicKey(username,
				trileadKey);
		if (!success)
			bridge.outputLine(manager.res.getString(
					R.string.terminal_auth_pubkey_fail, keyNickname));
		return success;
	}
	...
	private void finishConnection() {
		...
		for (PortForwardBean portForward : portForwards) {
			try {
				...
				bridge.outputLine(manager.res.getString(
						R.string.terminal_enable_portfoward,
						portForward.getDescription()));
			} catch (Exception e) {
		...
		if (!host.getWantSession()) {
			bridge.outputLine(manager.res
					.getString(R.string.terminal_no_session));
			bridge.onConnected();
			return;
		}
		...
		try {
			...
			Log.e(TAG,
					"Problem while trying to create PTY in finishConnection()",
					e1);
		}
	...
	public void connect() {
		...
		try {
			/*
			 * Uncomment when debugging SSH protocol: DebugLogger logger = new
			 * DebugLogger() {
			 *
	...
	public boolean enablePortForward(PortForwardBean portForward) {
		...
		if (HostDatabase.PORTFORWARD_LOCAL.equals(portForward.getType())) {
			...
			try {
				lpf = connection.createLocalPortForwarder(
						new InetSocketAddress(InetAddress.getLocalHost(),
								portForward.getSourcePort()), portForward
								.getDestAddr(), portForward.getDestPort());
			} catch (Exception e) {
			...
			return true;
		} else if (HostDatabase.PORTFORWARD_REMOTE
				.equals(portForward.getType())) {
			try {
				connection.requestRemotePortForwarding("",
						portForward.getSourcePort(), portForward.getDestAddr(),
			...
			portForward.setEnabled(true);
			return true;
		} else if (HostDatabase.PORTFORWARD_DYNAMIC5.equals(portForward
				.getType())) {
			DynamicPortForwarder dpf = null;

			try {
				dpf = connection
						.createDynamicPortForwarder(new InetSocketAddress(
								InetAddress.getLocalHost(), portForward
										.getSourcePort()));
			} catch (Exception e) {
			...
			// Unsupported type
			Log.e(TAG, String.format("attempt to forward unknown type %s",
					portForward.getType()));
			return false;
		}
	}
	...
	public boolean disablePortForward(PortForwardBean portForward) {
		...
		if (HostDatabase.PORTFORWARD_LOCAL.equals(portForward.getType())) {
			...
			if (!portForward.isEnabled() || lpf == null) {
				Log.d(TAG,
						String.format(
								"Could not disable %s; it appears to be not enabled or have no handler",
								portForward.getNickname()));
				return false;
			}
			...
			try {
				...
				Log.e(TAG,
						"Could not stop local port forwarder, setting enabled to false",
						e);
				return false;
			}
			...
			try {
				connection.cancelRemotePortForwarding(portForward
						.getSourcePort());
			} catch (IOException e) {
				Log.e(TAG,
						"Could not stop remote port forwarding, setting enabled to false",
						e);
				return false;
			}
			...
			try {
				...
				Log.e(TAG,
						"Could not stop dynamic port forwarder, setting enabled to false",
						e);
				return false;
			}
	...
	public static Uri getUri(String input) {
		...
		sb.append(PROTOCOL).append("://").append(Uri.encode(matcher.group(1)))
	...
	public String[] replyToChallenge(String name, String instruction,
		...
		for (int i = 0; i < numPrompts; i++) {
			...
			responses[i] = bridge.promptHelper.requestStringPrompt(instruction,
					prompt[i]);
		}
		return responses;
	}
	...
	public Map<String, byte[]> retrieveIdentities() {
		Map<String, byte[]> pubKeys = new HashMap<String, byte[]>(
				manager.loadedKeypairs.size());
	...
	public boolean addIdentity(Object key, String comment, boolean confirmUse,
			int lifetime) {
